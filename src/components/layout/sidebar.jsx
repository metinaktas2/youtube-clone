        ...
        <div className="py-3">
          {collapsedNavItems.map((item, key) => (
            <Link
              to={item.path}
              key={key}
              className="flex flex-col items-center justify-center py-4 px-2 mx-2 cursor-pointer rounded-lg transition hover:bg-grey"
            >
              ...
              <div>
                {category.items.map((item, key) => (
                  <Link
                    key={key}
                    to={item.path}
                    className="flex gap-6 items-center py-2 px-2 hover:bg-[#272727] rounded-lg"
                  >
