...
const Shorts = ({ shortList }) => {
  const validShorts = shortList?.data?.filter(
    (short) => short?.videoId && short?.thumbnail?.length
  );
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex gap-4 pb-4 py-4 overflow-x-auto scrollbar-hide">
        {validShorts?.map((short, key) => (
          <Link
            to={`/watch?v=${short.videoId}`}
            key={key}
            className="flex-shrink-0 group"
          >
            <div className="relative w-48 h-80 overflow-hidden rounded-lg bg-zinc-900">
              <img
                src={short.thumbnail?.[1]?.url || short.thumbnail?.[0]?.url}
