...
const Card = ({ video, isRow }) => {
  ...
  const thumbnail =
    isHover && video.richThumbnail?.length
      ...
      : null;
  const thumbnailUrl = thumbnail?.url || video.thumbnail?.[0]?.url || "";
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 flex flex-col gap-2 md:gap-4">
          ...
          <p className="test-sm text-[#aaaaaa] hover:text-white">
            {video.channelTitle}
          </p>

          <p className="line-clamp-1 text-[#aaaaaa] text-xs @md:text-sm">
            {video.description}
          </p>
        </div>
      </Link>
